-- Pro 2 — parita nutrition plan/constraints V1 + colonne operative Pro 2.
--
-- Fonti V1:
--   - `nextjs-empathy-pro/supabase/migrations/001_empathy_canonical_schema.sql`
--
-- Compatibilita Pro 2:
--   - `/api/nutrition/route.ts` legge `nutrition_plans.date`,
--     `kcal_target`, `carbs_g_target`, `proteins_g_target`,
--     `fats_g_target`, `hydration_ml_target`.
--   - `/api/nutrition/athlete-summary/route.ts` legge forma V1:
--     `from_date`, `to_date`, `goal`, `constraints_snapshot`.
...
-- Writes are usually service-role/API mediated, but authenticated owner/coach writes are allowed for compatibility.
