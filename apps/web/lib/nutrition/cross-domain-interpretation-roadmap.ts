...
export function buildCrossDomainInterpretationRoadmapV1(
  ...
  nodes.push({
    ...
    probeStatus: physReady ? "wired_deterministic" : "deferred_retrieval",
    summaryLineIt: physReady
    ...
    evidenceRefs: physReady ? ["physiology_state:current"] : [],
  });
  ...
  nodes.push({
    ...
    probeStatus:
      hormonalSignals.length || neurologicSignals.length || typeof twin?.readiness === "number"
        ? "active_stub"
        : "deferred_retrieval",
    summaryLineIt:
    ...
    evidenceRefs: [...hormonalSignals, ...neurologicSignals].slice(0, 4).map((_, i) => `pathway_level:${i}`),
  });

  nodes.push({
    domainId: "gut_microbiome",
    probeStatus: microbiotaSignals.length ? "wired_deterministic" : "deferred_retrieval",
    summaryLineIt: microbiotaSignals.length
    ...
    evidenceRefs: microbiotaSignals.slice(0, 6),
  });

  nodes.push({
    domainId: "genomics_nutrigenomics",
    probeStatus:
      geneticSignals.length || sessionKnowledge ? "active_stub" : "deferred_retrieval",
    summaryLineIt:
    ...
    evidenceRefs: geneticSignals.slice(0, 6),
  });

  nodes.push({
    domainId: "epigenetics",
    probeStatus: "deferred_retrieval",
    summaryLineIt:
