...
import { BIOENERGETIC_EVIDENCE_CURVE_CONTRACT_VERSION, type ExecutedWorkout, type PlannedWorkout } from "@empathy/contracts";
import {
  SIM_BANK_VERSION,
...
/** Slot giornalieri deterministici se il piano non ha orario (solo data). */
...
export function buildBioenergeticDayViewModelFromSlice(input: {
  ...
  const simGluLac =
    glucoseMeasured.length === 0 || lactateMeasured.length === 0
      ? buildSimulatedGluLacDiurnal(date, kernel, arbTl, {
          mealResponseScale01,
  ...
  const body: BioenergeticsDayViewModel = {
    ...
    disclaimers: [
      ...
      "Sim diurna glucosio/lattato: orari pasti dal diario; sedute eseguite usano started_at quando presente; piani senza orario usano fasce fisse deterministiche. Picchi post-prandiali scalati da contesto (stessa logica pesi fusione v1, senza LLM).",
    ],
    metricTiles,
    chart24h,
