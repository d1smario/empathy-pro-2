...
import { exchangeWhoopAuthorizationCode, fetchWhoopUserId } from "@/lib/integrations/whoop-oauth2-api";
...
/**
 * Callback OAuth2 WHOOP → salva token in `vendor_oauth_links` (service role).
 */
export async function GET(req: NextRequest) {
  ...
  const redirectErr = (reason: string, detailSnippet?: string) => {
    const u = profileWhoopUrl(req, "error", detailSnippet ?? reason);
  ...
  if (!redirectUri) {
    if (browserNav) return redirectErr("server_config", "WHOOP_OAUTH2_REDIRECT_URI");
    return NextResponse.json({ error: "WHOOP redirect URI non configurato." }, { status: 503 });
  }

  const tok = await exchangeWhoopAuthorizationCode({ code, redirectUri });
  if ("error" in tok) {
    if (browserNav) return redirectErr("token_exchange", tok.error);
    return NextResponse.json({ error: tok.error }, { status: 502 });
  }

  const externalUserId = await fetchWhoopUserId(tok.access_token);
  const expiresAt =
  ...
  if (browserNav) {
    return NextResponse.redirect(profileWhoopUrl(req, "ok").toString(), 302);
  }
  return NextResponse.json({ ok: true as const, vendor: "whoop" as const });
}
