...
import { exchangeSuuntoAuthorizationCode } from "@/lib/integrations/suunto-oauth2-api";
...
/** Callback OAuth2 Suunto → salva token (con refresh) in `vendor_oauth_links`. external_user_id = claim `user` del JWT. */
export async function GET(req: NextRequest) {
  ...
  if (!redirectUri) {
    if (browserNav) return redirectErr("server_config", "SUUNTO_OAUTH2_REDIRECT_URI");
    return NextResponse.json({ error: "Suunto redirect URI non configurato." }, { status: 503 });
  }

  const tok = await exchangeSuuntoAuthorizationCode({ code, redirectUri });
  if ("error" in tok) {
  ...
  return NextResponse.json({ ok: true as const, vendor: "suunto" as const });
}
