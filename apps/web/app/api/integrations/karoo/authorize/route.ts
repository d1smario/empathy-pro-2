...
import { KAROO_DEFAULT_SCOPE, karooAuthorizeUrl } from "@/lib/integrations/karoo-oauth2-api";
...
/**
 * Avvio OAuth2 Karoo (Hammerhead). Query: `athleteId`.
 * Env: `KAROO_OAUTH2_CLIENT_ID`, `KAROO_OAUTH2_REDIRECT_URI`, opz. `KAROO_OAUTH2_SCOPE`, `KAROO_OAUTH2_AUTHORIZE_URL`.
 */
export async function GET(req: NextRequest) {
  ...
  if (!clientId || !redirectUri) {
    ...
    return NextResponse.json(
      { error: "Karoo OAuth2 non configurato (KAROO_OAUTH2_CLIENT_ID / KAROO_OAUTH2_REDIRECT_URI)." },
      { status: 503 },
    );
  }

  const scope = process.env.KAROO_OAUTH2_SCOPE?.trim() || KAROO_DEFAULT_SCOPE;
  const authorize = new URL(karooAuthorizeUrl());
