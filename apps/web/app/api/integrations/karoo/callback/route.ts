...
import { exchangeKarooAuthorizationCode } from "@/lib/integrations/karoo-oauth2-api";
...
function profileUrl(req: NextRequest, code: string, detail?: string): URL {
  ...
  u.searchParams.set("karoo", code);
  if (detail) u.searchParams.set("detail", detail.slice(0, 500));
  return u;
}

/** Callback OAuth2 Karoo (Hammerhead) → salva token (con refresh) in `vendor_oauth_links` (vendor=hammerhead). */
export async function GET(req: NextRequest) {
  ...
  const redirectErr = (reason: string, detailSnippet?: string) => {
    const u = profileUrl(req, "error", detailSnippet ?? reason);
  ...
  const redirectUri = process.env.KAROO_OAUTH2_REDIRECT_URI?.trim();
  if (!redirectUri) {
    if (browserNav) return redirectErr("server_config", "KAROO_OAUTH2_REDIRECT_URI");
    return NextResponse.json({ error: "Karoo redirect URI non configurato." }, { status: 503 });
  }

  const tok = await exchangeKarooAuthorizationCode({ code, redirectUri });
  if ("error" in tok) {
  ...
  if (browserNav) return NextResponse.redirect(profileUrl(req, "ok").toString(), 302);
  return NextResponse.json({ ok: true as const, vendor: "hammerhead" as const });
}
