...
import { exchangePolarAuthorizationCode, registerPolarUser } from "@/lib/integrations/polar-oauth2-api";
...
/**
 * Callback OAuth2 Polar AccessLink → token (Basic), registrazione utente (`POST /v3/users`),
 * salvataggio in `vendor_oauth_links` (service role).
 */
export async function GET(req: NextRequest) {
  ...
  const redirectErr = (reason: string, detailSnippet?: string) => {
    const u = profilePolarUrl(req, "error", detailSnippet ?? reason);
  ...
  const redirectUri = process.env.POLAR_OAUTH2_REDIRECT_URI?.trim() || null;
  const tok = await exchangePolarAuthorizationCode({ code, redirectUri });
  if ("error" in tok) {
    if (browserNav) return redirectErr("token_exchange", tok.error);
    return NextResponse.json({ error: tok.error }, { status: 502 });
  }

  // Registrazione utente: obbligatoria prima di poter leggere i dati (409 = già registrato).
  const reg = await registerPolarUser({ accessToken: tok.access_token, memberId: athleteId });
  if ("error" in reg) {
    if (browserNav) return redirectErr("register_user", reg.error);
  ...
  const persisted = await upsertVendorOauthLink({
    athleteId,
    vendor: "polar",
    externalUserId: reg.polarUserId ?? tok.x_user_id,
    accessToken: tok.access_token,
    refreshToken: null,
    expiresAt,
    scope: process.env.POLAR_OAUTH2_SCOPES?.trim() || "accesslink.read_all",
  });
  ...
  if (browserNav) {
    return NextResponse.redirect(profilePolarUrl(req, "ok").toString(), 302);
  }
  return NextResponse.json({ ok: true as const, vendor: "polar" as const });
}
