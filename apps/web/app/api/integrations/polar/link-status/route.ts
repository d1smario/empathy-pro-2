...
/** Stato collegamento Polar (maschera `external_user_id`, nessun token). Auth + accesso atleta. */
export async function GET(req: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          code: error.code,
          ...(missingTable
            ? { hint: "Applica le migration vendor_oauth_links (037 + 068 Polar) e verifica SUPABASE_SERVICE_ROLE_KEY." }
            : {}),
        },
    ...
    const masked = !ext || ext.length <= 6 ? (ext ? "••••" : "—") : `${ext.slice(0, 3)}…${ext.slice(-4)}`;

    return NextResponse.json(
      {
        linked: true as const,
        polarUserIdMasked: masked,
