...
import { applyBiomechanicsStagingRun, rejectBiomechanicsStagingRun } from "@/lib/biomechanics/biomech-staging-apply";
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const body = (await req.json().catch(() => ({}))) as { reason?: string; calibration?: unknown };
    ...
    const result = await applyBiomechanicsStagingRun({
      db,
      athleteId,
      runId,
      userId,
    ...
    return NextResponse.json(
      {
        ok: true as const,
        runId,
        sessionImportId: result.sessionImportId,
        jobId: result.jobId,
    ...
    const message = err instanceof Error ? err.message : "biomech_staging_apply_failed";
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const athleteId = String(run.athlete_id ?? "");
    const { db } = await requireAthleteWriteContext(req, athleteId);
    const result = await rejectBiomechanicsStagingRun({ db, athleteId, runId, userId, reason });
    ...
    const message = err instanceof Error ? err.message : "biomech_staging_patch_failed";
    return NextResponse.json({ ok: false as const, error: message }, { status: 500, headers: NO_STORE });
  }
}
