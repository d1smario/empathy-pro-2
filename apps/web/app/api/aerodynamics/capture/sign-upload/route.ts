...
export async function POST(req: NextRequest) {
  try {
    ...
    if (fileSizeBytes > AERO_CAPTURE_MAX_BYTES) {
      return NextResponse.json({ error: "file_too_large" }, { status: 413, headers: NO_STORE });
    }
    if (!isAllowedAeroCaptureContentType(contentType)) {
    ...
    if (!admin) {
      return NextResponse.json(
        { error: "aero_capture_requires_SUPABASE_SERVICE_ROLE_KEY" },
        { status: 503, headers: NO_STORE },
      );
    }

    const safe = sanitizeAeroCaptureObjectName(fileName);
    const objectPath = `${athleteId}/${randomUUID()}_${safe}`;
    const signed = await admin.storage.from(AERO_CAPTURE_BUCKET).createSignedUploadUrl(objectPath);
    ...
    return NextResponse.json(
      { ok: true, bucket: AERO_CAPTURE_BUCKET, path, token, objectPath: path },
      { headers: NO_STORE },
    );
  } catch (err) {
    ...
    const message = err instanceof Error ? err.message : "aero_sign_upload_failed";
