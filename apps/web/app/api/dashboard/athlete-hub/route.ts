...
/**
 * Fase 5 — hub dashboard: sintesi atleta in una sola round-trip (stesso accesso degli altri moduli).
 */
export async function GET(req: NextRequest) {
  const client = createSupabaseCookieClient();
  if (!client) {
    return NextResponse.json(
      { ok: false as const, error: "supabase_unconfigured" },
  ...
  const [
    ...
    client
      ...
      .gte("date", from)
      .lte("date", to),
    client.from("nutrition_constraints").select("*").eq("athlete_id", athleteId).maybeSingle(),
    client
